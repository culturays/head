...
{/* flex justify-center flex-col sm:flex-row items-center p-1 */}
  {!closeQuestion&& 
        <form method="POST" action="https://content.culturays.com/wp-json/wp/v2/char"className="flex flex-col md:flex-row md:flex-wrap lg:flex-nowrap md:p-4 md:w-full md:justify-center items-center  p-1" onSubmit={submitForm}>
         <div className="flex flex-col "> 
            <label className="font-bold p-2 m-1">Character :</label>
            <input type="text" className="p-3 text-sm m-2 focus:outline-none border rounded border-green-500 text-black" placeholder="New Character Here" name="title" onChange={handleInput} value={formData.title} />
          </div> 
          <div className="flex flex-col"> 
            <label className="font-bold p-2 m-1">Film Title:</label>
            <input type="text"className="p-3 text-sm m-2 focus:outline-none border rounded border-green-500 text-black" placeholder="What Film Do They Appear In" name="content" onChange={handleInput} value={formData.content} />
          </div>
          <div className="mt-10"> 
