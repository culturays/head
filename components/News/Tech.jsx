 ...
 setCurrPg(pageNumber) 
  ...
  const replaceHTMLTags=(string)=>{
    ...
    return newString
     ...
     const title_item=tech_news.map((ex)=>ex.contentTypeName)[0]  
  ...
  return (
    ...
    <div className="xl:flex justify-center gap-1 px-2 m-auto" style={{maxWidth:'1550px'}}>  
      ...
      {tech_items.length>4 &&tech_items.slice(4,8).map((xy,i)=>
      <div className="max-w-md lg:max-w-xl m-auto" key={xy?.title + ' ' + i}>
        ...
        <div className="my-3 cursor-pointer ">
          <Link href={`/topic/${xy?.contentTags?.nodes[0]?.slug}/${xy?.contentTags?.nodes[0]?.id}`}></Link> <span className="border rounded-2xl bg-red-500 text-white p-2 hover:bg-red-600">
           ...
           <Link href={`/news/tech/${xy?.slug}`}><h2 className="text-xl font-medium hover:text-gray-500">{xy?.title}</h2></Link> 
           </div>
...
<div>
<h2 className="text-5xl font-bold m-4">Tech</h2>
...
<Link href={`/news/tech/${xy?.slug}`}><h2 style={{ display: '-webkit-box', WebkitLineClamp:2, WebkitBoxOrient: 'vertical' }} className="overflow-hidden text-ellipsis  leading-8 text-xl py-1 mt-4 hover:text-gray-600 cursor-pointer">{xy?.title} </h2></Link>
<Link href={`/news/tech/${xy?.slug}`}><p style={{ display: '-webkit-box', WebkitLineClamp:2, WebkitBoxOrient: 'vertical' }} className="overflow-hidden text-ellipsis my-2 hover:text-gray-600 text-base">{replaceHTMLTags(xy?.excerpt)} </p></Link>
 <small className="text-sm my-3 text-red-500 "><em>{moment(xy?.date).fromNow()}</em></small> 
</div>
)}
</div>
