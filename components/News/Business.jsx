...
const Business = ({business_news}) => {
  const top_business_items= business_news.slice(0,4) 
  const carousel_business_items= business_news.slice(0,4) 
...
<section className='xl:my-20 px-2 border border-yellow-700 w-10/12 xl:w-full m-auto bg-gray-800 my-4'>
 ...
 <div className="mx-11"> 
 { carousel_business_items.map((item, index)=> 
 index===activeSlide&&
 <div className='my-2' key={item.name + ' ' + index}> 
  <Link href={`/news/business/${item.slug}`}><h2 className='text-3xl text-gray-300 my-1 font-bold hover:text-gray-200 cursor-pointer'>{item?.title} </h2> </Link>
  <Link href={`/news/business/${item.slug}`}><p className='m-1 text-base text-gray-200 hover:text-gray-50 cursor-pointer'>{replaceHTMLTags(item.excerpt)} </p></Link>
...
<Link href={`/news/business/${xy.slug}`}><h2 className="text-3xl mt-4 w-3/4 hover:text-gray-400">{xy.title} </h2></Link>
</div>
<Image
...
/> 
<Link href={`/news/business/${xy.slug}`}><p className="my-2 text-lg hover:text-gray-500">{replaceHTMLTags(xy.excerpt)} </p></Link>
 <small className="text-sm my-3 text-red-500"><em>{moment(xy.date).fromNow()}</em></small> 
</div>
)}
</div>
...
<div>
{activeSet&&business_news.map((xy, ix)=> 
<div className="my-2 px-2 m-auto border-b xs:w-3/4 sm:w-full" key={xy.title + ' ' + ix}>
