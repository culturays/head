...
const Society = ({society_news }) => {

  const [posts, setPosts]=useState([]) 
  const [currPg, setCurrPg]=useState(1)
  const [postPerPage, setPostPerP]=useState(10) 
     const world_news = society_news.map((ex)=>ex.societyCategories?.nodes.filter((xy)=> xy?.name==="World")).flat().map((tx)=> tx?.societies.nodes).flat()
     const africa_news = society_news.map((ex)=>ex.societyCategories?.nodes.filter((xy)=> xy?.name==="Africa")).flat().map((tx)=> tx?.societies.nodes).flat()     
     const society_items=society_news.map((ex)=>ex.societyCategories?.nodes.filter((xy)=> xy?.name!=="World")).flat().filter((xy)=> xy?.name!=="Africa").map((tx)=> tx?.societies.nodes).flat()
  function decrement() {
   setCurrPg(currPg - 1);
 }
 function increment() {
 ...
 useEffect(()=>{
   const fetchPs= async()=>{ 
     setPosts([...society_items.slice(8)]) 
   }
   fetchPs()
 },[society_news])
 ...
 setCurrPg(pageNumber) 
  ...
  const replaceHTMLTags=(string)=>{
    ...
    return newString
     ...
     const title_item=society_news.map((ex)=>ex.contentTypeName)[0]   
  ...
  return (
    ...
    <div className="xl:flex justify-center gap-1 px-2 m-auto" style={{maxWidth:'1550px'}}>  
      ...
      {society_items.length>4 &&society_items.slice(4,8).map((xy,i)=>
      <div className="max-w-md lg:max-w-xl m-auto" key={xy?.title + ' ' + i}>
        ...
        <div className="my-3 cursor-pointer ">
          <Link href={`/topic/${xy?.contentTags?.nodes[0]?.slug}/${xy?.contentTags?.nodes[0]?.id}`}></Link> <span className="border rounded-2xl bg-red-500 text-white p-2 hover:bg-red-600">
           ...
           <Link href={`/news/society/${xy?.slug}`}><h2 className="text-xl font-medium hover:text-gray-500">{xy?.title}</h2></Link> 
           </div>
...
<div className="max-w-2xl m-auto xl:m-0">
  <SlideFxn title_item={title_item} content={society_items}/>  
</div>
...
<div>
<h2 className="text-5xl font-bold m-4">Society</h2>
...
<Link href={`/news/society/${xy?.slug}`}>
...
/> 
<Link href={`/news/society/${xy?.slug}`}><p style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}className="overflow-hidden text-ellipsis my-2 text-lg hover:text-gray-500 py-2 leading-9">{replaceHTMLTags(xy?.excerpt)} </p></Link>
 <small className="text-sm my-3 text-red-500"><em>{moment(xy?.date).fromNow()}</em></small> 
</div>
)}
</div>
...
<Link href={`/news/society/${xy?.slug}`}><h2 style={{ display: '-webkit-box', WebkitLineClamp:2, WebkitBoxOrient: 'vertical' }} className="overflow-hidden text-ellipsis  leading-8 text-xl py-1 mt-4 hover:text-gray-600 cursor-pointer">{xy?.title} </h2></Link>
<Link href={`/news/society/${xy?.slug}`}><p style={{ display: '-webkit-box', WebkitLineClamp:2, WebkitBoxOrient: 'vertical' }} className="overflow-hidden text-ellipsis my-2 hover:text-gray-600 text-base">{replaceHTMLTags(xy?.excerpt)} </p></Link>
 <small className="text-sm my-3 text-red-500 "><em>{moment(xy?.date).fromNow()}</em></small> 
</div>
)}
</div>
