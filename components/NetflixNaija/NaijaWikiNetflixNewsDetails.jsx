...
const NaijaWikiNetflixNewsDetails =({content_videos,news_details}) => {
  ...
  const replaceHTMLTags=(string)=>{
    ...
    return newString
     ...
     const related_content=news_details.netflixNewsGroup.netflixRelated  
 ...
 <SlidingSide newsItems={content_videos} speed={60000}/> 
      ...
      <div className="bg-white my-2 py-10 rounded-lg mx-4 xs:mx-8 sm:m-auto sm:w-4/5">
       ...
       <p className="p-4 text-end text-lg bg-gray-700 text-gray-200 mx-4 xs:mx-20 bg-opacity-70"><small className="italic p-3"> by</small>{ news_details.author.node.name } </p> 
...
/>  
 ...
 <p className="p-4 mt-10 text-lg">{replaceHTMLTags(news_details.excerpt)} </p> 
  ...
  <div key={ex + ' ' + Math.random()} className="bg-black py-4 first:border-b first:sm:border-b-0 first:sm:border-r px-3"> 
  <div className="flex"> 
