...
return (
  <> 
<div className="top_event mt-8 m-auto xl:w-4/5 lg:w-11/12 px-14 min-[481px]:px-8" >
...
/>
 <ul> 
 <Link href={`/naija-events/event/${ux.slug }`}><li title='view event'className="absolute bottom-16 cursor-pointer text-white hover:text-gray-200 px-3 py-2 z-10 font-bold text-lg underline">{ux.title }</li></Link>
 </ul>
</div>  
)}

 <h2 className="absolute top-16 text-cnter cursor-pointer text-white px-3 py-2 z-10 font-bold text-2xl">{ex.charAt(0).toUpperCase() + ex.slice(1).replace(/-/g," ") }</h2> 
</div> 
) }  
)}   
</div> 
...
<div className="top_event mt-8 m-auto xl:w-4/5 lg:w-11/12 px-14 min min-[481px]:px-8" >
