...
const Events = ({user,setActive, active, events}) => {
 const openForm = () => {
        setActive(prev => !prev); 
    }  
...
<div className="sm:flex-row flex flex-col w-1/2 justify-center items-center sm:w-3/4 sm:justify-between md:w-1/2 md:justify-evenly m-auto">  
 <Image 
className="rounded-xl mt-5"
src={xx.img_url &&xx.img_url.endsWith('.jpg')||ux.img_url.endsWith('.jpeg')||xx.img_url.endsWith('.png')?`https://peezrwllibppqkolgsto.supabase.co/storage/v1/object/public/event_avatars/${xx.img_url}`:'/assets/images/culturays_events.png'}
width={150} 
height={150} 
alt={xx.title}
/>  
...
<p className="text-white mx-0.5 text-md mt-1">Date: {xx?.day.match(dateRegex)[0] ||'See Details'}</p>
<p className="text-white mx-0.5 text-md mt-1">Time: {xx?.day.match(timeRegex)[0] ||'See Details'}</p>
</div>
</div>
