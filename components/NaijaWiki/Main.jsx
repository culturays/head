...
return newString
 ...
 <Link href={`/naija-wiki/character/${it.slug }`}><p className='overflow-hidden text-ellipsis  hover:text-gray-200 leading-relaxed text-lg cursor-pointer'style={{ display: '-webkit-box', WebkitLineClamp:3, WebkitBoxOrient: 'vertical' }}>{replaceHTMLTags(it.excerpt)} </p></Link>   
 ...
 <section className='lg:flex relative m-auto ' style={{maxWidth:'1550px'}}> 
  ...
  <div key={nt.slug} className='border border-orange-400'>
    ...
    <Link href={`/netflix-naija/news/${nt.slug}`}><h2 className='xs:text-2xl md:text-4xl py-2'> {nt.title}</h2></Link>
    <Link href={`/netflix-naija/news/${nt.slug}`}><p className='sm:text-lg py-2 hidden sm:block'>{replaceHTMLTags(nt.excerpt)}</p ></Link> 
...
<div className='lg:w-1/2 lg:mx-1 sm:grid sm:grid-cols-2 gap-1 border border-orange-400'>
  ...
  <div key={nt.slug} className='[&:nth-child(3)]:sm:border-r [&:nth-child(3)]::md:border-r-0 first:sm:border-r first:md:border-r-0 home_grid relative border-b lg:border-b-0 lg:border-t border-orange-400 lg:text-white lg:relative'>
    ...
    <Link href={`/netflix-naija/news/${nt.slug}`}><h2 className='overflow-hidden text-ellipsis hover:md:text-gray-200 text-xl underline lg:no-underlinelg:text-xl'style={{ display: '-webkit-box', WebkitLineClamp:3, WebkitBoxOrient: 'vertical' }}> {nt.title}</h2></Link>
...
<li className='list-disc text-lg p-3' key={index}> <a target='_blank'href={` ${itx.url}`} >{itx.title }</a></li>
 ...
 <li key={index} className='list-disc text-lg p-3'><a href={` ${itx.url}`} >{itx.title }</a></li>
  ...
  <section className='bg-white py-8 px-4'>
<h2 className='text-3xl font-bold my-6 text-center text-gray-700'>News</h2> 
<hr className='m-4'/>   
...
<div className='flex flex-col sm:flex-row font-bold sm:justify-between items-center my-6 text-slate-800 text-white border-b px-8'>
 ...
 <Link href={`/news/video/${it.slug }`} ><div dangerouslySetInnerHTML={{__html:it.excerpt}} className='text-gray-300 text-base m-2'/></Link> 
     </div>
    </div> )}  
...
<Link href={`/topic/${item.contentTags.nodes[0].slug}`}><h4 className=' my-3 hover:text-gray-500 text-red-600'>&#8212; {item.contentTags.nodes[0].name }</h4></Link> 
 ...
 <div className='bg-white my-8 py-8'> 
    <h2 className='text-3xl font-bold my-6 text-center text-slate-800 opacity-80'>Africa</h2> 
...
<Link href={`/topic/${item.contentTags.nodes[0].slug}`}><h4 className=' py-1 font-bold'>&#8212; {item.contentTags.nodes[0].name }</h4></Link> 
 ...
 <div className='lg:flex bg-white my-1 py-6 justify-center px-6 gap-4'> 
<section className=''>
